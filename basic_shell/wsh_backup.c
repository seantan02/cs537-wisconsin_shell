...
To-Do:
- [x] Add batch mode
- [x] Remember to use access() to check if executable can be found before concluding it's error
- [x] Check error code for argc>2
- [x] Check if it's a built-in call
- [x] Implement each built-in call
- [x] Implement redirection '>' correctly
- [] Implement '&>' correctly
- [] Check for mem leak
*/
	...
	if(pointerToStartOfRedirection != NULL && pointerToStartOfRedirection[0] == '>'){
	  ...
	  *pointerToStartOfRedirection = '>'; // add > back
	  ...
	  if(pointerToStartOfRedirection[1] == '>'){ // 
		...
		file = open(outputFileName, O_WRONLY|O_CREAT, 0666);
